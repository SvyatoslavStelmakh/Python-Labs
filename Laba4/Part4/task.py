...
monthly_stats = df.groupby(df['дата покупки'].dt.to_period('M')).agg({
    'сумма': ['count', 'sum', 'mean'],
    'тип пассажиров': 'count'
}).round(0)

monthly_stats.columns = ['Количество транзакций', 'Суммарная выручка', 'Средняя выручка', 'Количество пассажиров']
print(monthly_stats)
...
print("Наибольшая прибыль наблюдается в период с ИЮЛЯ по АВГУСТ")
